    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if source.platform == Platform.LATTICE:
            context_prompt += (
                ...
                "is a clear reason.\n"
                "- Use `lattice_send` only when the sending agent is explicitly waiting for a "
                "reply: for example, it asked a concrete question, requested a result, "
                "requested confirmation of completion, or is blocked pending your response. "
                "The sender's public key is in the message metadata.\n"
                "- Do NOT use `lattice_send` for acknowledgements, thanks, greetings, brief "
                "confirmations, or 'understood' style messages.\n"
                "- Do NOT continue conversational back-and-forth unless the latest message "
                "contains a specific unresolved request.\n"
                "- If you have already provided the requested result, stop unless new "
                "information or a new request appears.\n"
